...
function withSenderName(db, row) {
...
export function registerNudgeRoutes(router, ctx) {
  ...
  router.post('/api/nudges', async ({ res, body, me }) => {
    ...
    if (!areFriends(db, me.id, target.id)) throw ApiError.forbidden('You can only nudge your friends');
    ...
    sendJson(res, 201, { nudge: withSenderName(db, row) });
  }, { auth: true });

  router.post('/api/nudges/:id/accept', async ({ res, params, me }) => {
    const nudge = db.prepare('SELECT * FROM nudges WHERE id = ?').get(params.id);
    ...
    const at = nowISO();
    db.prepare("UPDATE nudges SET status = 'accepted', accepted_at = ? WHERE id = ?").run(at, nudge.id);
    ...
    const fresh = db.prepare('SELECT * FROM nudges WHERE id = ?').get(nudge.id);
    sendJson(res, 200, { nudge: withSenderName(db, fresh) });
  }, { auth: true });
}
